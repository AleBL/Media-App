...
function getGenreNames(ids, language) {
  if (!ids || !Array.isArray(ids) || ids.length === 0) {
    return "";
  }

  return ids.map(id => {
    const genre = GENRES[language].find(genre => genre.id === id);
...
// Data obtained from endpoint -> https://api.themoviedb.org/3/configuration/languages?api_key=<ADD TMDB API KEY HERE>
